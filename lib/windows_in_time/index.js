...
class Earth {
  ...
  async setTimes(date) {
    ...
    // Cookies.remove('days')
    if( days ){
      var cookie_day = _.find( Cookies.getJSON('days'), (d) => { return DateTime.fromISO( d.sunrise ).toFormat('yyyy MM dd') == date.toFormat('yyyy MM dd') })
      if( cookie_day ){
    ...
    var url = ['https://api.sunrise-sunset.org/json?lat=', this.lat, '&lng=', this.lng, '&date=', day.toISODate(),'&formatted=0'].join('')
  ...
  day(date){
    ...
    var times = SunCalc.getTimes(date, this.lat, this.lng);
    ...
    if( DateTime.now() < sunrise ){
      ...
      times = SunCalc.getTimes(date, this.lat, this.lng);
